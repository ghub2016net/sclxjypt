   ...
   public class AjaxLoginLog
    {
        ...
        /// <returns></returns>
        [Action]
        public static string DeleteUserLog(string param)
        {
            if (Int32.Parse(DataCache.GetCache(HzsKey.CACHE_HTTYPE).ToString()) <= 1)
            {
                ...
                try
                {
                    y = DbHelper.ExecuteNonQuery("delete from [LoginLog]  where " + zhi.Substring(0, zhi.LastIndexOf("or")), null, CommandKind.SqlTextNoParams);
                    if (y <= 0)
