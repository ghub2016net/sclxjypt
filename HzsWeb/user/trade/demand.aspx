<%@ Page Language="C#" AutoEventWireup="true" CodeFile="demand.aspx.cs" Inherits="user_trade_demand" %>
...
<body>   
<div class="mainRighTitle mainRighTitleMessageIco">供求信息管理 > 需求信息列表 <span style=" text-align:right; float:right; padding-right:20px;"><select class="selectList10" id="ddlProperty" onchange="getddl()"><option value="10">审核通过</option><option value="20">待审核</option><option value="40">审核未通过</option></select></span></div>
...
<%if (molist.Count == 0)
  ...
  <%}
    ...
    <td><a class="mainIco mainEdit" href="demand_edit.aspx?action=Edit&id=<%=molist[i].id %>">编辑</a><a class="mainIco mainIcoSearch" target="_blank" href="/trade/detail.aspx?id=<%=molist[i].id %>">查看</a><a class=" mainIco mainDelete" href="javascript:del(<%=molist[i].id %>);">删除</a></td>
  </tr>
  <% } }%>
